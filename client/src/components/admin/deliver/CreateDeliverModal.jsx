...
import { createDeliver } from '../../../store/admin/deliver-slice';
import { X, DollarSign, FileText } from 'lucide-react';

export default function CreateDeliverModal({ isOpen, onClose }) {
  ...
  const handleClose = () => {
    setFormData({ deliverFee: '', description: '' });
    setErrors({});
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-sm shadow-xl w-full max-w-lg">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-400">
          <h2 className="text-lg font-semibold text-gray-900">Thêm phí vận chuyển</h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded-sm outline-none focus:ring-1 focus:ring-slate-800 ${
                errors.deliverFee ? 'border-red-500' : 'border-slate-400'
              }`}
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200">
            ...
            >
              {isLoadingActions ? 'Đang tạo...' : 'Tạo phí vận chuyển'}
            </button>
          </div>
        </form>
      </div>
